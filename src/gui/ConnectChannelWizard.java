...
public class ConnectChannelWizard extends WizardBase {
	...
	public ConnectChannelWizard() { super();
	
		page0=new VBox();
			pg0_help=new Label(L10n.t("ConnectChannelHelp0")); pg0_help.requestFocus();
			pg0_separator1 = new Separator(Orientation.HORIZONTAL);
			pg0_alias = new TextField(); pg0_alias.setPromptText(L10n.t("Channel Name"));
			pg0_p2sh = new TextField(); pg0_p2sh.setPromptText(L10n.t("Сhannel Public Address"));
		...
		page0.getChildren().addAll(pg0_help,pg0_separator1,pg0_alias,pg0_p2sh,pg0_region,pg0_separator2,pg0_next);
		pg0_next.setOnAction((ev)->{
			
			final String account=pg0_alias.getText();
			final String p2sh=pg0_p2sh.getText();

			if(addressBookController!=null) {
				jobInfo.progress("Blockchain scanning..."); pg0_next.setDisable(true); 
				
				Threads.runNow(()->{
					StringResponse adr=rpcCommander.importAddress(p2sh.trim(), account!=null ? account: "");
					...
					Platform.runLater(()->{
						if(error==null) {
							...
							pg1_p2sh.setText(this.p2sh); pg1_p2sh.selectAll();
							this.next();
						}
						else {
							jobInfo.alert(error); 
		...
		page1=new VBox();
			pg1_help=new Label(L10n.t("ConnectChannelHelp1")); pg1_help.requestFocus();
			pg1_separator1 = new Separator(Orientation.HORIZONTAL);
			
			pg1_p2sh = new TextField(); pg1_p2sh.setEditable(false); 
			pg1_p2sh.selectedTextProperty().addListener((obs, oldSelect, newSelect)->{ if(!this.p2sh.equals(newSelect)) pg1_p2sh.selectAll(); });
	...
	public ConnectChannelWizard(Parent owner) { this(); 
		this.setOwner(owner);	
	}
